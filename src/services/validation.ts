...
export const GrowthRecordSchema = z.object({
  ...
  weight: z.number().min(0, 'Weight cannot be negative').max(50, 'Weight must be less than 50kg').optional(),
  length: z.number().min(0, 'Length cannot be negative').max(200, 'Length must be less than 200cm').optional(),
  head_circumference: z.number().min(0, 'Head circumference cannot be negative').max(100, 'Head circumference must be less than 100cm').optional(),
  note: z.string().max(500, 'Note must be 500 characters or less').optional(),
