...
export const TimelineRow = memo(
  ({ event, onEdit, onDelete }: TimelineRowProps) => {
    const getIcon = () => {
      switch (event.type) {
        case 'feed':
    ...
    return (
      ...
      >
        <div className='flex-shrink-0 flex items-center gap-4 flex-1 min-w-0' onClick={onEdit}>
          <div className='flex-shrink-0'>{getIcon()}</div>

          <div className='flex-1 min-w-0 space-y-1'>
            <div className='text-[17px] leading-[24px] font-medium truncate'>{getTitle()}</div>
            {event.note && (
              <Badge variant='secondary' className='text-[13px] leading-[18px] font-normal'>
                {event.note}
              </Badge>
            )}
            <div className='text-[14px] leading-[20px] text-text-subtle'>{getTimeDisplay()}</div>
          </div>
        </div>
