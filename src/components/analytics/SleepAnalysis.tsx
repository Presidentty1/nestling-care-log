...
export function SleepAnalysis({ babyId, dateRange }: SleepAnalysisProps) {
  ...
  const avgSleepDuration =
    completedSleeps.length > 0 ? totalSleepHours / completedSleeps.length : 0;
  const avgSleepPerDay =
    completedSleeps.length > 0 ? totalSleepHours / (dateRange === 'week' ? 7 : 30) : 0;

  // Prepare chart data
  const chartData = completedSleeps.map(sleep => ({
    date: new Date(sleep.startTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    hours: (
      (new Date(sleep.endTime).getTime() - new Date(sleep.startTime).getTime()) /
      (1000 * 60 * 60)
    ).toFixed(1),
  }));
