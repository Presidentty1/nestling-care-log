...
export default function ManageCaregivers() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!currentFamilyId) {
        ...
        const familyName = firstBaby?.name ? `${firstBaby.name}'s Family` : 'My Family';
      ...
      const transformedMembers = (membersData || []).map((m: any) => ({
        ...
        role: m.role,
        profiles:
          Array.isArray(m.profiles) && m.profiles.length > 0 ? m.profiles[0] : m.profiles || null,
      }));
  ...
  const handleInviteClick = () => {
    if (!isPro) {
      toast.error(
        'Caregiver invites require Nestling Pro. Upgrade to invite partners and sync across devices.'
      );
      navigate('/settings');
      return;
    }
      ...
      if (error.message?.includes('404') || error.message?.includes('FunctionsRelayError')) {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : 'Failed to send invite. Please check the email address and try again.';
            ...
            <div className='flex items-start gap-3'>
              ...
              <div>
                ...
                <p className='text-sm text-muted-foreground'>
                  Invite partners, family, or nannies. One Pro subscription covers everyone in your
                  baby's care team.
                </p>
              </div>
            </div>
              ...
              <div className='flex items-start gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-xs text-muted-foreground mb-3'>
                    Invite partners and family members to sync logs in real-time. Requires Nestling
                    Pro.
                  </p>
          ...
          <div className='px-4 py-6 space-y-6'>
            ...
            <div className='space-y-2'>
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value='admin' className='text-[17px]'>
                    Admin (full access)
                  </SelectItem>
                  <SelectItem value='member' className='text-[17px]'>
                    Member (can log)
                  </SelectItem>
                  <SelectItem value='viewer' className='text-[17px]'>
                    Viewer (read only)
                  </SelectItem>
                </SelectContent>
              </Select>
