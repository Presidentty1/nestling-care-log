...
import { EventType, NapPrediction, Baby } from '@/types/events';
...
export default function Home() {
  ...
  const loadSummary = async () => {
    if (!activeBabyId) return;
    const todaySummary = await dataService.getTodaySummary(activeBabyId);
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        ...
        <div>
          ...
          {events.length > 0 ? (
            <EventTimeline
              events={events}
              onEdit={(event) => setModalState({ open: true, type: event.type as EventType, editingId: event.id })}
