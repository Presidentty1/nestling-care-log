...
export default function AnalyticsDashboard() {
  ...
  function calculateAnalytics(
    ...
    const tummyTimeMinutes = tummyTimes.reduce((sum, e) => {
      ...
      if (e.start_time && e.end_time) {
        return (
          sum + (new Date(e.end_time).getTime() - new Date(e.start_time).getTime()) / (1000 * 60)
        );
      }
      return sum;
    }, 0);
    ...
    const sleepPatterns: { hour: number; duration: number }[] = Array.from(
      { length: 24 },
      (_, i) => ({
        hour: i,
        duration: 0,
              ...
              <ResponsiveContainer width='100%' height={300}>
                <LineChart data={analyticsData.trends}>
                  ...
                  <Line
                    type='monotone'
                    dataKey='feeds'
                    stroke='hsl(var(--event-feed))'
                    name='Feeds'
                  />
                  <Line
                    type='monotone'
                    dataKey='sleep'
                    stroke='hsl(var(--event-sleep))'
                    name='Sleep (h)'
                  />
                  <Line
                    type='monotone'
                    dataKey='diapers'
                    stroke='hsl(var(--event-diaper))'
                    name='Diapers'
                  />
                </LineChart>
              </ResponsiveContainer>
