...
class AuthViewModel: ObservableObject {
    ...
    @Published var passwordResetSent = false // Track if password reset email was sent
    ...
    private func saveSession(_ session: Session) async throws {
        ...
        guard status == errSecSuccess else {
            throw NSError(domain: "KeychainError", code: Int(status), userInfo: [
                NSLocalizedDescriptionKey: "Failed to save session to Keychain"
            ])
        }
    }
    ...
    private func loadSession() async throws -> Session? {
        ...
        guard status == errSecSuccess, let data = result as? Data else {
            ...
            throw NSError(domain: "KeychainError", code: Int(status), userInfo: [
                NSLocalizedDescriptionKey: "Failed to load session from Keychain"
            ])
        }
    ...
    private func clearSession() async throws {
        ...
        if status != errSecSuccess && status != errSecItemNotFound {
            throw NSError(domain: "KeychainError", code: Int(status), userInfo: [
                NSLocalizedDescriptionKey: "Failed to clear session from Keychain"
            ])
        }
    }
