        ...
        .safeAreaInset(edge: .bottom) {
            VStack(spacing: 8) {
                Button(action: primaryAction) {
                    Text(primaryTitle)
                        .font(.body)
                        .foregroundColor(primaryDisabled ? NuzzleTheme.textSecondary : NuzzleTheme.primaryForeground)
                        .frame(maxWidth: .infinity, minHeight: 44)
                        .padding(.spacingMD)
                        .background(primaryDisabled ? NuzzleTheme.surface : NuzzleTheme.primary)
